...
class FW_Extension_Integrations extends FW_Extension {

	private $post_type = 'fw-integrations';
	private $slug = 'project';
	private $taxonomy_slug = 'integrations';
	private $taxonomy_name = 'fw-integrations-category';
	private $taxonomy_tag_name = 'fw-integrations-tag';
	private $taxonomy_tag_slug = 'integrations-tag';
	...
	private function add_admin_actions() {
		...
		add_action( 'restrict_manage_posts', array( $this, '_action_admin_add_integrations_edit_page_filter' ) );
		// listing screen
	...
	private function save_permalink_structure() {
		...
		$post = FW_Request::POST( 'fw_ext_integrations_project_slug',
			apply_filters( 'fw_ext_integrations_post_slug', $this->slug )
		);

		$taxonomy = FW_Request::POST( 'fw_ext_integrations_integrations_slug',
			apply_filters( 'fw_ext_integrations_taxonomy_slug', $this->taxonomy_slug )
		);
	...
	public function _action_add_permalink_in_settings() {
		add_settings_field(
			'fw_ext_integrations_project_slug',
			__( 'Project base', 'fw' ),
		...
		add_settings_field(
			'fw_ext_integrations_integrations_slug',
			__( 'Integrations category base', 'fw' ),
			array( $this, '_integrations_slug_input' ),
			'permalink',
			'optional'
		);
	}
	...
	public function _project_slug_input() {
		?>
		<input type="text" name="fw_ext_integrations_project_slug" value="<?php echo $this->slug; ?>">
	 ...
	 * @internal
	 */
	public function _integrations_slug_input() {
		?>
		<input type="text" name="fw_ext_integrations_integrations_slug" value="<?php echo $this->taxonomy_slug; ?>">
		<code>/my-integrations</code>
		<?php
	}

	public function add_admin_filters() {
		add_filter( 'parse_query', array( $this, '_filter_admin_filter_integrations_by_integrations_category' ), 10, 2 );
	...
	public function _action_register_post_type() {
		...
		$post_names = apply_filters( 'fw_ext_projects_post_type_name',
			array(
				'singular' => __( 'Integrations project', 'fw' ),
				'plural'   => __( 'Integrations', 'fw' )
			) );

		register_post_type( $this->post_type,
			array(
				'labels'             => array(
					'name'               => $post_names['plural'], //__( 'Integrations', 'fw' ),
					'singular_name'      => $post_names['singular'], //__( 'Integrations project', 'fw' ),
				...
				'description'        => __( 'Create a integrations item', 'fw' ),
				'public'             => true,
				'show_ui'            => true,
				'show_in_menu'       => true,
				...
				'has_archive'        => false,
				'rewrite'            => array(
					'slug' => 'demos', 'with_front' => false
				),
				'menu_position'      => 4,
				'show_in_nav_menus'  => true,
				'menu_icon'          => 'dashicons-integrations',
				'hierarchical'       => false,
				'query_var'          => true,
	...
	public function _action_register_taxonomy() {
		...
		$category_names = apply_filters( 'fw_ext_integrations_category_name', array(
		...
		register_taxonomy( $this->taxonomy_name, $this->post_type, array(
			'labels'            => array(
				'name'              => sprintf( _x( 'Integrations %s', 'taxonomy general name', 'fw' ), $category_names['plural'] ),
				'singular_name'     => sprintf( _x( 'Integrations %s', 'taxonomy singular name', 'fw' ), $category_names['singular'] ),
		...
		if ( apply_filters('fw:ext:integrations:enable-tags', false) ) {
			$tag_names = apply_filters( 'fw_ext_integrations_tag_name', array(
				'singular' => __( 'Tag', 'fw' ),
				'plural'   => __( 'Tags', 'fw' )
			) );
	 ...
	 * @internal
	 */
	public function _action_admin_add_integrations_edit_page_filter() {
		...
		if ( empty( $terms ) || is_wp_error( $terms ) ) {
			echo '<select name="' . $this->get_name() . '-filter-by-integrations-category"><option value="0">' . __( 'View all categories',
					'fw' ) . '</option></select>';

			return;
		}

		$get = FW_Request::GET( $this->get_name() . '-filter-by-integrations-category' );
		$id  = ( ! empty( $get ) ) ? (int) $get : 0;

		$dropdown_options = array(
			'selected'        => $id,
			'name'            => $this->get_name() . '-filter-by-integrations-category">',
	 ...
	 * @return WP_Query
	 */
	public function _filter_admin_filter_integrations_by_integrations_category( $query ) {
		$screen = fw_current_screen_match( array(
			'only' => array(
				'base'      => 'edit',
				'id'        => 'edit-' . $this->post_type,
		...
		$filter_value = FW_Request::GET( $this->get_name() . '-filter-by-integrations-category' );
